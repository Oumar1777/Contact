...
/**
 * @brief GestionContact::getContactAt Récuperer le contact à la position donnée
 * @param place : la position du contact
 * @return Le contact à la position place.
 * @warning Si aucun contact à cette position, le contact retourne aura un id = NULL
 */
Contact GestionContact::getContactAt(const int place)
{
    Contact contact; contact.setID("NULL");
    auto it = listContact.begin();
    int i = 0;
    while(it != listContact.end())
    {
        if(i == place)
            return *it;
        else
...
/**
 * @brief GestionContact::getContactAdr Cette fonction retourne l'adresse dans la liste d'un contact
 * @param idContact: l'identifiant du contact à trouver
 * @return l'adresse du contact dans la liste
 */
Contact* GestionContact::getContactAdr(const QString & idContact)
{
